...
if($q == 'addtocart'){
    ...
    $jim->addtocart($product,$price,$qty , $maxQty , $productId , $productBrand, $productCategory);
}else if($q == 'emptycart'){
    unset($_SESSION['cart']); 
...
class Shopping {
    function addtocart($product, $price, $qty , $maxQty , $productId , $productBrand, $productCategory){
        $cart = array(
    ...
    function checkout(){
        ...
        foreach($_SESSION['cart'] as $row):
            if($row['qty'] != 0){
                ...
                $newQty = ((int) $row['maxQty'] - (int)$row['qty']);
                $product = '('.$row['qty'].') '.$row['product'].'-'.$productBrand.'-'.$productCategory;
                $item = $product.', '.$item;
                $productId = $row['productId'];

                $q = "UPDATE products set qty = '$newQty' where ID = '$productId'";
                mysql_query($q);
            }
        endforeach;
