...
var d=document.forms["addproduct"]["brand"].value;
if (d==null || d=="")
  {
  alert("Pls Select an Brand");
  return false;
  }
...
<form action="addexec.php" method="post" enctype="multipart/form-data" name="addproduct" onsubmit="return validateForm()">
 ...
 Price<br />
  ...
  Quantity<br />
<input name="quantity" type="number" min="1" value="1" class="ed" onkeypress="return isNumberKey(event)" /><br />
 Category<br />
...
?>
    ...
    Brand<br />
    <select name="brand" class="ed">

<?php
    include('db.php');
    $r = mysql_query("select * from brands");
    while($row = mysql_fetch_array($r)){
         echo '<option>'.$row['title'].'</option>';
    }
?>
